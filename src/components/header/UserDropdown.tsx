...
import { logoutInAppSlice } from "@/redux/slices/appSlice";
...
export default function UserDropdown() {
  ...
  const { loggedInUserDetails } = useSelector((state: any) => state.app);
  ...
  const userAvatar = userDetails?.avatar || loggedInUserDetails?.avatar;
  const userName = userDetails?.name || loggedInUserDetails?.name;
  const userEmail = userDetails?.email || loggedInUserDetails?.email;
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      showToast(
        "error",
        errData?.title || "Request Failed",
        errData?.message || "Logged out Failed"
      );
    } finally {
      setLoading(false);
    }
