...
import CommonTable, { HeaderType } from "../common/CommonTable";
...
interface TooltipFontConfig {
  size: number;
  weight: "normal" | "bold" | "lighter" | "bolder" | number; // More specific type
}
...
export default function MainComponent() {
  ...
  useEffect(() => {
    ...
    const commonTooltipConfig: CommonTooltipConfig = {
      ...
      bodyFont: {
        size: 13,
        weight: "normal", // Changed to more specific value
      },
      callbacks: {
    ...
    // Productivity Chart
    const productivityCtx = document.getElementById(
      "productivityChart"
    ) as HTMLCanvasElement;
    if (productivityCtx) {
      new Chart(productivityCtx.getContext("2d")!, {
        type: "line",
        data: {
    ...
    if (taskDistCtx) {
      new Chart(taskDistCtx.getContext("2d")!, {
        type: "doughnut",
        data: {
    ...
    if (holdCtx) {
      new Chart(holdCtx.getContext("2d")!, {
        type: "bar",
        data: {
          labels: ["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct"],
          datasets: [
